...
void send_msg(char type, int dest, int length, char* payload, struct sockaddr_in* sockstr, int len_sock) {
  ...
  *ite = type; // FYI MYM END etc
  ...
  memcpy(ite, payload, length);
	  ...
	  // Specific UDP
  sendto(dest, (const char *)msg, size_msg, 0, (struct sockaddr *) sockstr, len_sock); 
